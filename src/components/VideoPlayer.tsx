...
interface VideoPlayerProps {
  ...
  poster?: string;
  autoplay?: boolean;
  audioUrl?: string; // Companion audio commentary stream if available
}

// Convert numbers to Arabic glyphs index
...
// Formatter for elapsed media times in Arabic
...
// Safe URL parsing & cors proxying
const getProxiedUrl = (url: string | undefined): string => {
  if (!url) return "";
  let cleanUrl = url.trim();
  
  // Normalizing faulty headers
...
export function VideoPlayer({ src, poster, autoplay = true, audioUrl }: VideoPlayerProps) {
  ...
  const [brightness, setBrightness] = useState(1.0); // Simulated hardware brightness gradient

  // Media Level Specifications (Adaptive Streaming Levels)
  const [levels, setLevels] = useState<any[]>([]);
  const [activeLevel, setActiveLevel] = useState<number>(-1); // -1 is AUTO
  const [audioTracks, setAudioTracks] = useState<any[]>([]);
  const [activeAudioTrack, setActiveAudioTrack] = useState<number>(-1);
  const [captionsTracks, setCaptionsTracks] = useState<any[]>([]);
  const [activeCaptionsTrack, setActiveCaptionsTrack] = useState<number>(-1); // -1 is OFF

  // Gesture Controls & Touch States
  ...
  // Auto Reconnection & Resiliency States
  ...
  // Manage secondary companion audio (commentary stream) synchronization if present
  ...
  // Update dynamic captions/subtitle tracks
  ...
  // Playback initialization engine
  const initPlayer = useCallback(() => {
    ...
    if (isHlsUrl && Hls.isSupported()) {
      ...
      // MPEG-DASH stream loading via dashjs
      ...
      // Safari OS native fallback HLS player to optimize RAM usage
      video.src = resolvedSrc;
      video.addEventListener("loadedmetadata", () => {
        if (autoplay) video.play().catch(() => {});
      });
    } else {
      // Generic MP4, MKV, Mov streams
      video.src = resolvedSrc;
    }
    ...
    // Attach text tracker listeners for subtitles detection
    video.addEventListener("loadedmetadata", handleSubtitleTracksUpdate);
    if (video.textTracks) {
      video.textTracks.addEventListener("addtrack", handleSubtitleTracksUpdate);
    }

    return () => {
      video.removeEventListener("loadedmetadata", handleSubtitleTracksUpdate);
      if (video.textTracks) {
        video.textTracks.removeEventListener("addtrack", handleSubtitleTracksUpdate);
      }
    };
  }, [resolvedSrc, autoplay, volume, isMuted, playbackRate, useProxy, handleSubtitleTracksUpdate]);

  const handleFatalFailover = (reason: string) => {
    if (retryCount < maxRetries) {
      setIsReconnecting(true);
      setRetryCount(prev => prev + 1);
      triggerToast(`اتصال متذبذب. يتم فحص جودة السيرفر... (${retryCount + 1}/${maxRetries})`);
      setTimeout(() => {
        initPlayer();
      }, 2000);
    } else {
  ...
  // Continuous high-precision zero-drift audio-video synchronization loop (every 100ms)
  useEffect(() => {
    ...
    const syncInterval = setInterval(() => {
      ...
      // Ensure audio play/pause states are matched exactly
      ...
      // If playtime deviates by more than 150ms, snap them back to absolute sync instantly
      const drift = Math.abs(video.currentTime - audio.currentTime);
      if (drift > 0.15 && !video.seeking) {
  ...
  const wakeControls = () => {
    ...
    if (isPlaying && !showSettings && !isLocked) {
      ...
      }, 2000); // 2 seconds auto-hide strictly matching TV interface standard specifications
    }
  };
  ...
  const handleVolumeChange = (val: number) => {
    ...
    if (audioRef.current) {
      ...
      audioRef.current.muted = cleanVol === 0 || isMuted;
    }
  };
  ...
  const chooseQuality = (levelId: number) => {
    if (hlsRef.current) {
      ...
      if (levelId === -1) {
        ...
        (dashRef.current as any).updateSettings({
          streaming: {
  ...
  // Chromecast streaming link simulation
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    const deltaY = touchStartRef.current.y - touch.clientY; // swipe upwards is positive
    ...
    // Determine swipe type on first move
    ...
    if (gType === 'seek' && video && duration) {
      ...
      const secondsToSeek = seekRatio * 180; // Scale 180 seconds across width
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    if (durationSinceStart < 250 && deltaX < 8 && deltaY < 8) {
      const parentObj = containerRef.current as any;
      const now = Date.now();
      const lastTap = parentObj?.lastTap || 0;
  ...
  // Keyboard navigation mappings (optimal for remote settings & Smart TV devices)
  ...
  // Custom aspect ratio layout utility classes
  ...
  return (
    ...
    >
      ...
      {/* Floating Center Play Button - Only displayed when paused */}
      ...
      {/* Buffering/Loading spinning HUD panel */}
      ...
      {/* Floating Chromecast Active Status indicator */}
      {isCasting && (
      ...
      {/* 4. MAIN OTT CONTROLS BAR SYSTEM (Glassmorphic Floating Frame) */}
      <div 
      ...
      >
        {/* TOP RAIL: Back Trigger & Sports Badges */}
        ...
        {/* Dynamic High-Contrast Audio & Commentary Synced Status Indicators */}
        ...
        {/* MIDDLE RAIL: Clean centered play indicator when visible */}
        <div className="flex justify-center items-center gap-5 my-auto">
          {/* Transparent minimal controls without large blocking screens */}
        </div>

        {/* BOTTOM METADATA RAIL: Clean floating layout with essential controls */}
        <div className="flex justify-between items-center max-w-[95%] mx-auto w-full pointer-events-auto gap-3">
          {/* Right side: Play/Pause and Volume controls */}
          <div className="flex items-center gap-2">
            {/* Play/Pause Button */}
            <button 
              onClick={togglePlay}
          ...
          {/* Left side: Rotate and Fullscreen controls */}
          <div className="flex items-center gap-2">
            ...
            {/* Fullscreen Player Option Button */}
            <button 
        ...
        {/* 5. SLIDE-OUT PANEL MENU SETTINGS (OTT Interactive tabs menu overlay) */}
        {showSettings && (
          <div 
              ...
              <div className="space-y-1 pr-1">
                {[0.5, 0.75, 1, 1.25, 1.5, 2].map((rate) => (
                  ...
                  >
                    ...
                    {playbackRate === rate && <span className="w-1.5 h-1.5 bg-[#00C2FF] rounded-full shadow-[0_0_8px_#00C2FF]" />}
                  </button>
                ))}
              </div>
            )}

            {/* CORS / Anti Block Debug Toggle */}
            {settingsTab === "help" && (
              <div className="space-y-3 pt-1 text-right font-medium text-xs leading-relaxed text-gray-400 pr-1">
                <div className="bg-black/20 p-2.5 rounded-2xl border border-white/5">
                  <span className="text-[#F5F7FA] font-bold block mb-1">تشفير حماية تدفق الفيديو</span>
                  <div className="grid grid-cols-2 gap-2 mt-1.5">
                    <button
                      onClick={() => {
                        setUseProxy(true);
                        triggerToast("تم تشغيل سيرفر الحماية البروكسي 🔐");
                        initPlayer();
                      }}
                      className={`py-1.5 rounded-xl text-[10px] font-black transition-all text-center ${useProxy ? "bg-[#00C2FF] text-slate-950" : "bg-white/5 text-gray-400"}`}
                    >
                      🔐 مفعل (بروكسي)
                    </button>
                    <button
                      onClick={() => {
                        setUseProxy(false);
                        triggerToast("تم تفعيل بروتوكول الاتصال المباشر ⚡");
                        initPlayer();
                      }}
                      className={`py-1.5 rounded-xl text-[10px] font-black transition-all text-center ${!useProxy ? "bg-[#00C2FF] text-slate-950" : "bg-white/5 text-gray-400"}`}
                    >
                ...
                <div className="flex gap-2 text-[10px] items-start px-1 font-bold">
                  <Wifi className="w-4 h-4 text-[#00C2FF] shrink-0 mt-0.5 animate-pulse" />
                  <p>يدعم مشغل Stad المتقدم تسريع العتاد وجداول الدك لتجنب انقطاع البث تحت أسوأ ظروف الشبكات.</p>
                </div>
              </div>
            )}
          </div>
